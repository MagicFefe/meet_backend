...
def validate_image(image: str):
    try:
        ...
        raise InvalidImageError(detail="image_path must be jpeg or png format")
    width, height = decoded_image.size
    if width != height:
        raise InvalidImageError(detail="image_path must be a square")
    if width < MIN_USER_IMAGE_WIDTH_PX or height < MIN_USER_IMAGE_HEIGHT_PX:
        raise InvalidImageError(detail="image_path is too small")
    if width > MAX_USER_IMAGE_WIDTH_PX or height > MAX_USER_IMAGE_HEIGHT_PX:
        raise InvalidImageError(detail="image_path is to large")
